...
xlim <- list(
  min = min(na.omit(data$HI_INC_DEBT_MDN)) - 500,
  max = max(na.omit(data$LO_INC_DEBT_MDN)) + 500
)
ylim <- list(
  min = min(na.omit(data$MN_EARN_WNE_P6))-500,
  max = max(na.omit(data$MN_EARN_WNE_P10)) + 500
)
